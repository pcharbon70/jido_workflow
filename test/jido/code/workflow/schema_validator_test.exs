defmodule Jido.Code.Workflow.SchemaValidatorTestCustomStep do
  use Jido.Action,
...
defmodule Jido.Code.Workflow.SchemaValidatorTestCustomTrigger do
  use GenServer
...
defmodule Jido.Code.Workflow.SchemaValidatorTest do
  ...
  test "validate_workflow/1 rejects whitespace-only signals.topic and return.value" do
    ...
    assert Enum.any?(errors, fn error ->
             error.path in [["signals"], ["signals", "topic"]]
           end)

    assert Enum.any?(errors, fn error ->
             error.path in [["return"], ["return", "value"]]
           end)
  end
  ...
  test "validate_workflow/1 supports custom plugin step types" do
    assert :ok =
             PluginExtensions.register_step_type(
               "custom_step",
               Jido.Code.Workflow.SchemaValidatorTestCustomStep
             )
  ...
  test "validate_workflow/1 supports custom plugin trigger types" do
    assert :ok =
             PluginExtensions.register_trigger_type(
               "webhook",
               Jido.Code.Workflow.SchemaValidatorTestCustomTrigger
             )
