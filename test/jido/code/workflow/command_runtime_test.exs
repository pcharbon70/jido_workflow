defmodule Jido.Code.Workflow.CommandRuntimeTestActions.Echo do
  use Jido.Action,
...
defmodule Jido.Code.Workflow.CommandRuntimeTestActions.DelayedValue do
  use Jido.Action,
...
defmodule Jido.Code.Workflow.CommandRuntimeTestActions.EchoWithIdProbe do
  use Jido.Action,
...
defmodule Jido.Code.Workflow.CommandRuntimeTest do
  ...
  test "does not echo missing requested_reason in workflow.run.cancel.rejected payloads",
    ...
    assert {:ok, _published} =
             Bus.publish(context.bus, [
               Signal.new!(
                 "workflow.run.cancel.requested",
                 %{"run_id" => "run_done_without_reason"},
                 source: "/test/client"
               )
             ])
  ...
  test "echoes invalid atom mode in workflow.run.mode.rejected payloads", context do
    assert {:ok, _published} =
             Bus.publish(context.bus, [
               Signal.new!(
                 "workflow.run.mode.requested",
                 %{"run_id" => "run_mode_invalid_atom", "mode" => :manual},
                 source: "/test/client"
               )
             ])
    ...
    assert String.contains?(reason, "mode")
  end
  ...
  test "echoes atom limit filters in workflow.definition.list.rejected payloads", context do
    assert {:ok, _published} =
             Bus.publish(context.bus, [
               Signal.new!(
                 "workflow.definition.list.requested",
  ...
  defp write_id_probe_workflow(dir, workflow_name) do
    ...
    ### echo
    - **type**: action
    - **module**: Jido.Code.Workflow.CommandRuntimeTestActions.EchoWithIdProbe
    - **inputs**:
      - value: `input:value`
      - id: `input:id`
  ...
  defp write_strategy_workflow(dir, workflow_name) do
    ...
    ### delayed
    - **type**: action
    - **module**: Jido.Code.Workflow.CommandRuntimeTestActions.DelayedValue
    - **inputs**:
      - value: `input:value`
