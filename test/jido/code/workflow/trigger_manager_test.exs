defmodule Jido.Code.Workflow.TriggerTestActions.Echo do
  use Jido.Action,
    name: "trigger_echo",
    schema: [
...
defmodule Jido.Code.Workflow.TriggerManagerTest do
  ...
  test "sync_from_registry returns invalid_trigger_config errors for malformed trigger config",
    ...
    assert Enum.any?(errors, &match?(%Jido.Code.Workflow.ValidationError{}, &1))
  end
  ...
  defp write_trigger_workflow(dir, name) do
    ...
    ### echo
    - **type**: action
    - **module**: Jido.Code.Workflow.TriggerTestActions.Echo
    - **inputs**:
      - value: `input:value`
