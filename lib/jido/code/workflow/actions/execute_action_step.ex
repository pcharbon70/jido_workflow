defmodule Jido.Code.Workflow.Actions.ExecuteActionStep do
  @moduledoc """
  ...
  def run(%{step: step} = params, _context) do
    ...
    with {:ok, step_meta} <- normalize_step(step) do
      ...
      case execute_step_action(step_meta, state) do
        {:ok, result} ->
  ...
  defp execute_step_action(step_meta, state) do
    with {:ok, target_module} <- resolve_module(step_meta.module),
         {:ok, resolved_inputs} <- ArgumentResolver.resolve_inputs(step_meta.inputs, state) do
      run_action(target_module, resolved_inputs, step_meta)
    end
  end
    ...
    cond do
      ...
      true ->
        {:ok,
         %{
           name: to_string(name),
           type: normalize_step_type(fetch(step, "type"), "action"),
           module: module,
           inputs: inputs,
